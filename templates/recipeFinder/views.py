...
# Type in ingredients you want to find recipes for
...
# suggestions based on what's in the inventory
def stockSuggestions(request):
	ingredients = InventoryItem.objects.order_by('?')
	search_phrase = ''
	...
	context = None
	while context is None and len(ingredients) >= 3:
		context = get_search_results(request, ingredients, search_phrase)
		ingredients = ingredients[:-1]
